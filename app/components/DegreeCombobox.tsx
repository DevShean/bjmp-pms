...
type DegreeComboboxProps = {
  value: string;
  onValueChange: (val: string) => void;
  placeholder?: string;
};

export default function DegreeCombobox({ value, onValueChange, placeholder = "Select or type degree" }: DegreeComboboxProps) {
  ...
  const filtered = useMemo(() => {
    ...
    if (!norm) return DEGREE_OPTIONS;
    return DEGREE_OPTIONS.filter(opt => opt.toLowerCase().includes(norm));
  }, [query]);
