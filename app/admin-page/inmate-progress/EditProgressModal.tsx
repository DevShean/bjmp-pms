...
export default function EditProgressModal({ isOpen, onClose, onSave, record }: EditProgressModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent 
        className="flex w-full max-w-lg flex-col rounded-2xl bg-white shadow-2xl overflow-hidden border-none p-0"
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-teal-100 flex items-center gap-1.5 mt-0.5 uppercase tracking-wider font-medium text-left">
                Inmate: {localRecord?.inmateName || "Select an Inmate"}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="px-6 pt-4 pb-5 space-y-4">
          ...
          <div className="rounded-xl bg-slate-50 border border-slate-200 p-4">
            ...
            <p className="font-lexend font-semibold text-slate-800 text-left">{localRecord?.program || "N/A"}</p>
          </div>
